...
import StringIO, sys, getopt, re, string, urllib

def _myCap(text):
    def commonWords(x):
        if x.lower() in ['in', 'and', 'of']: return x.lower()
        else: return string.capitalize(x)
...
class Publication:
	...
	if self.startPage == '': self.startPage = 0
	else: 
            try:
	...
	if self.volume in ['submitted',  'in press', 'ASAP']: return False
	else: return True
	...
	for first, last in self.authors:
		...
		authors.append('<span class="lead">%s, %s</span>' % (last, first))
	    else: authors.append('%s, %s' % (last, first))
...
def _genLatexEntry(pub, color, id=None):
	...
	if last.find(pub.myself) != -1:
	    authors.append('\\textbf{%s, %s}' % (last, first))
            ...
            if color:
                authors.append('\\textcolor[rgb]{0.6, 0.6, 0.6}{%s, %s}' % (last, first))
            else:
	...
	entry = """\\entry*[%s] \\raggedright{%s; ``%s'', %s, \\textbf{%s}, %s}""" % \
	    (labelstr, authors, pub.title, pub.formatJournal('latex'), pub.year, pub.volume)
    else:
	entry = """\\entry*[%s] \\raggedright{%s; ``%s'', %s, \\textbf{%s}, \\textit{%s}, %s}""" % \
	    (labelstr, authors, pub.title, pub.formatJournal('latex'), pub.year, pub.volume, pub.pages.replace('-', '--'))
...
def _genWikiEntry(pub):
    ...
    authors = []
    for first, last in pub.authors: authors.append('%s, %s' % (last, first))
    authors = '; '.join(authors)
    entry += authors+'<br>'
    ...
    entry += "''%s'', '''%s''', ''%s'',  %s %s" % \
        (_myCap(pub.journal).replace("\\", ""), pub.year, pub.volume, pub.pages, url)
    return entry
...
if __name__ == '__main__':
	...
	else: abstract = abstract[0].text

        kwds = record.findall('keywords/keyword')
        if not kwds: kwds = None
        elif kwds[0].text == None: kwds = None
        else: kwds = [x.strip() for x in kwds[0].text.split(';')]

        title = title.encode('ascii', 'ignore')
        
	p = Publication(authors, title, journal, year, volume, pages, kwds, url, abstract)
        p.setMyself(myself)
	pubs.append(p)
